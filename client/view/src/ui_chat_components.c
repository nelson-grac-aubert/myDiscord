...
void components_draw_modal(SDL_Renderer *renderer, ChatLayout *layout, TTF_Font *font_title, TTF_Font *font_main, TTF_Font *font_sub, int mx, int my, SDL_Color white_color, SDL_Color gray_color, SDL_Color dark_gray) {
    // 1. Dessin de l'arrière-plan semi-transparent (Overlay)
    SDL_Rect overlay = {0, 0, layout->window_w, layout->window_h};
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 160);
    SDL_RenderFillRect(renderer, &overlay);

    // 2. Dessin de la boîte de dialogue (Modal)
    ...
    // 3. Zone de saisie de texte (Input)
    ...
    // 4. Interrupteur Salon Privé (Toggle)
    ...
    // 5. Bouton Annuler (Changement de couleur de texte au survol)
    ...
    // 6. Bouton Confirmer / Créer (Changement de couleur de fond au survol)
